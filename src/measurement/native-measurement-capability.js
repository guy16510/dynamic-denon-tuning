export const NATIVE_MEASUREMENT_STATES = Object.freeze(['unsupported', 'calibration-only', 'pre-correction-only', 'post-correction-capable', 'unknown']);

function rmsDelta(a, b) {
  if (!Array.isArray(a) || !Array.isArray(b) || !a.length || a.length !== b.length) return null;
  let deltaEnergy = 0, referenceEnergy = 0;
  for (let i = 0; i < a.length; i += 1) {
    const av = Number(a[i]), bv = Number(b[i]);
    if (!Number.isFinite(av) || !Number.isFinite(bv)) return null;
    deltaEnergy += (av - bv) ** 2; referenceEnergy += av ** 2;
  }
  const delta = Math.sqrt(deltaEnergy / a.length), reference = Math.sqrt(referenceEnergy / a.length);
...
export function compareCorrectionExperiment({ multEqOff, multEqOn, sameMicPosition, sameSpeaker, sameStimulus, expectedCorrectionMaterial = true, threshold = 0.01 }) {
  ...
  return { validControl: blockers.length === 0, changed, relativeRmsDelta, threshold, blockers, interpretation: blockers.length ? 'inconclusive' : changed ? 'native capture contains a material OFF/ON difference; this supports but does not alone prove post-correction capability' : 'native capture is effectively unchanged; treat it as pre-correction-only until contrary hardware evidence exists' };
}
...
export function evaluateNativeMeasurementCapability(evidence = {}) {
  ...
  if (evidence.calibrationCapture === true && evidence.normalPlaybackCapture !== true) return capability('calibration-only', evidence, 'Capture is proven only inside the Audyssey calibration workflow.');
  if (evidence.normalPlaybackCapture === true && evidence.correctionExperiment) {
    ...
    if (experiment.validControl && experiment.changed === true && evidence.activePresetVerified === true && evidence.capturePathVerified === true) return capability('post-correction-capable', { ...evidence, correctionExperimentResult: experiment }, 'Controlled hardware evidence supports active post-correction capture.');
...
function capability(state, evidence, reason) {
  const hardwareVerification = state === 'post-correction-capable' ? 'hardware-verified' : state === 'unknown' ? 'hardware-unverified' : evidence.source === 'configuration' ? 'software-tested' : 'hardware-verified';
  return Object.freeze({ state, postCorrection: state === 'post-correction-capable', usableForSearch: ['calibration-only', 'pre-correction-only', 'post-correction-capable'].includes(state), usableForPhysicalVerification: state === 'post-correction-capable', hardwareVerification, reason, evidence });
}
