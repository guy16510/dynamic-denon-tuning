...
server.tool('measurement_capture_new', 'Complete or adopt exactly one manual REW measurement and persist its immutable evidence.', { sessionId: z.string(), position: z.number().int().min(0).max(6), channel: z.string(), beforeMeasurementKeys: z.array(z.string()), shieldFile: z.string().optional(), expectedPreset: z.number().int().min(1).max(2).optional(), measurementType: z.string().optional() }, guarded(async args => measurement.captureManual(args)));
