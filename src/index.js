...
server.tool('theater_autotune_finalize_verification', 'Legacy manual-score finalizer retained for compatibility. Prefer theater_autotune_finalize_measured, which derives metrics from parent-owned immutable REW evidence.', { sessionId: z.string(), baselineMetrics: z.record(z.number()), candidateMetrics: z.record(z.number()), changes: z.array(z.record(z.any())).default([]), remainingIssues: z.array(z.union([z.string(), z.record(z.any())])).default([]), minimumGain: z.number().min(0).max(20).default(0.5), majorRegression: z.number().min(1).max(30).default(8) }, guarded(async args => theater.finalizeVerification(args)));
...
server.tool('measurement_capture_new', 'Complete or adopt exactly one manual REW measurement and persist its immutable evidence. Verification/proof callers must preserve negotiated measurement settings.', { sessionId: z.string(), position: z.number().int().min(0).max(6), channel: z.string(), beforeMeasurementKeys: z.array(z.string()), shieldFile: z.string().optional(), expectedPreset: z.number().int().min(1).max(2).optional(), measurementType: z.string().optional(), measurementSettings: z.object({ command: z.string().nullable().optional(), playbackMode: z.string().nullable().optional(), measurementMode: z.string().nullable().optional(), stimulus: z.string().nullable().optional() }).optional() }, guarded(async args => measurement.captureManual(args)));
...
server.tool('calibration_compare_sessions', 'Derive and compare two measured sessions without accepting either one. Matched coverage is reported explicitly.', { baselineSessionId: z.string(), candidateSessionId: z.string(), minimumGain: z.number().min(0).max(20).default(0.5), majorRegression: z.number().min(1).max(30).default(8) }, guarded(async ({ baselineSessionId, candidateSessionId, minimumGain, majorRegression }) => {
  const [baselineRecords, candidateRecords] = await Promise.all([sessions.readMeasurementRecords(baselineSessionId, { acceptedOnly: true }), sessions.readMeasurementRecords(candidateSessionId, { acceptedOnly: true })]);
  const coverage = validateMatchedCoverage(baselineRecords, candidateRecords);
  ...
  return { coverage, baseline, candidate, comparison: compareScores(baseline.score, candidate.score, { minimumGain, majorRegression }), missing: { baseline: missingDerivedMetrics(baseline.derived), candidate: missingDerivedMetrics(candidate.derived) } };
}));
server.tool('calibration_compare_scores', 'Apply the acceptance rule to supplied component scores. This does not substitute for measured preset verification.', { baselineMetrics: z.record(z.number()), candidateMetrics: z.record(z.number()), minimumGain: z.number().min(0).max(20).default(0.5), majorRegression: z.number().min(1).max(30).default(8) }, guarded(async ({ baselineMetrics, candidateMetrics, minimumGain, majorRegression }) => { const baseline = scoreCalibration(baselineMetrics); const candidate = scoreCalibration(candidateMetrics); return { baseline, candidate, comparison: compareScores(baseline, candidate, { minimumGain, majorRegression }) }; }));
